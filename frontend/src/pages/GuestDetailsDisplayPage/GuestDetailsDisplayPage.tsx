...
export default function GuestDetailsDisplayPage() {
  ...
  const bookingRisk = guest.aiAnalysis?.bookingRisk || {
    ...
    probability: 0,
    reasons: [
      "This guest was created before booking-risk analysis was enabled.",
    ],
    recommendation:
      "Open AI preference analysis after the guest profile is analysed again.",
    history: {
      repeatGuest: false,
      previousBookings: 0,
  ...
  const updateBookingStatus = async (bookingStatus: string) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8000/api/guests/${guest._id}/booking-status`,
        {
          method: "PATCH",
  ...
  return (
    <main className="guest-detail-page">
      <header>
        <div className="guest-detail-navigation">
          <Link to="/Hotelstaffdashboard">← Staff dashboard</Link>
          <Link to="/guest-review-queue">Guest review queue</Link>
        </div>
        ...
        <label className="booking-status-control">
          ...
          <select
            value={guest.bookingStatus || "Pending"}
            disabled={savingBookingStatus}
            onChange={(event) => updateBookingStatus(event.target.value)}
          >
      ...
      <section className={`booking-risk-card ${riskClass}`}>
        <div className="booking-risk-main">
          <span className="booking-risk-icon">
            <ShieldAlert size={24} />
          </span>
          <div>
            ...
            <p>
              Use this signal to decide the right staff follow-up before
              arrival.
            </p>
          </div>
        </div>
        <div className="risk-score">
          <strong>{bookingRisk.probability}%</strong>
          <span>{bookingRisk.level} risk</span>
        </div>
        <div className="risk-details">
          <div>
            <h3>Why this signal?</h3>
            <ul>
              {(bookingRisk.reasons || []).map((reason: string) => (
                <li key={reason}>
          ...
          <div className="risk-recommendation">
            <span>Recommended staff action</span>
            <b>{bookingRisk.recommendation}</b>
          </div>
        </div>
        <div className="risk-history">
          <span>
            <b>{bookingRisk.history?.previousBookings || 0}</b> previous
            bookings
          </span>
          <span>
            <b>{bookingRisk.history?.completedStays || 0}</b> completed stays
          </span>
          <span>
            <b>{bookingRisk.history?.previousCancellations || 0}</b> previous
            cancellations
          </span>
        </div>
      </section>
